...
export function SentimentChart() {
  ...
  const [mentionCount, setMentionCount] = useState<string>('0');
  const dashboardService = DashboardService.getInstance();
  const { filter } = useFilter();

  useEffect(() => {
    loadSentiment();
    // Generate mention count on client side to avoid hydration mismatch
    setMentionCount((Math.random() * 10000 + 5000).toFixed(0));
        ...
        <div className="space-y-4">
          ...
          <div className="pt-4 border-t">
            ...
            <div className="mt-2 text-xs text-muted-foreground">
              Based on {mentionCount} analyzed mentions
            </div>
          </div>
        </div>
      </CardContent>
