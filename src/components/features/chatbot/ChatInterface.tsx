...
export function ChatInterface({ initialMessage }: ChatInterfaceProps) {
  ...
  const handleSendMessage = async (messageText?: string) => {
    ...
    try {
      const botResponse = await chatbotService.sendMessage({ message: text });
      const endTime = Date.now();
      setResponseTime(endTime - startTime);
      
      // Map ChatResponse -> ChatMessage for UI
      const botMessage: ChatMessage = {
        ...
        chart: botResponse.chart
          ? { type: 'bar', data: botResponse.chart.data, title: botResponse.chart.title }
          : undefined,
      };
